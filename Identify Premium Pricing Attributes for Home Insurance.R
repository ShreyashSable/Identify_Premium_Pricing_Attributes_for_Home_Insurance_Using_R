...
insurance <- insurance %>%
  mutate(
    column1 = na_if(column1, ""),
    column2 = na_if(column2, "")
  )
...
#all_equal <- all(guide_loc == panel_loc, na.rm = TRUE)

#Now lets check for payment frequency with premium amount
...
#we found that client who having Ownership type 3 are mostly like to pay more premium.
